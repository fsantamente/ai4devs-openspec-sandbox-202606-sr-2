...
// *********************************************************************//
  IBrowserInfo = interface(IDispatch)
    ...
    procedure SetCookieEx(const Name, Value, Expires, Path, Options: WideString); safecall;
...
// *********************************************************************//
  IBrowserInfoDisp = dispinterface
    ...
    procedure SetCookieEx(const Name: WideString; const Value: WideString; const Expires: WideString;
      const Path: WideString; const Options: WideString); dispid 210;
  end;
...
var
  DllSetRpcPrefix: procedure(APrefix: PChar); stdcall;
  DllSetRpcPrefixA: procedure(APrefix: PAnsiChar); stdcall;
...
var
  reg: TRegistry;
  key32, key64, aux, tmp32, tmp64: string;
  DevMode: Boolean;
  Ini: TIniFile;
begin
  aux := '';
...
{$ENDIF}
  ...
  begin
    ...
    try
      ...
      if not (reg.OpenKeyReadOnly(tmp32) or reg.OpenKeyReadOnly(tmp64)) then
      begin
        tmp32 := '\SOFTWARE\Wow6432Node\Cybele Software\Setups\Thinfinity\Workspace';
        tmp64 := '\SOFTWARE\Cybele Software\Setups\Thinfinity\Workspace';
        if reg.OpenKeyReadOnly(tmp32) or reg.OpenKeyReadOnly(tmp64) then
        begin
          key32 := tmp32;
          key64 := tmp64;
        end else begin
          tmp32 := '\SOFTWARE\Wow6432Node\Cybele Software\Setups\Thinfinity\Workspace HyC';
          tmp64 := '\SOFTWARE\Cybele Software\Setups\Thinfinity\Workspace HyC';
          if reg.OpenKeyReadOnly(tmp32) or reg.OpenKeyReadOnly(tmp64) then
          begin
            key32 := tmp32;
            key64 := tmp64;
    ...
    if FileExists(GetModulePath + 'OEM.ini') then
    begin
 ...
 if LibHandle = 0 then
  begin
    ...
    try
      ...
      @DllSetRpcPrefix := GetProcAddress(LibHandle, 'DllSetRpcPrefix');
      @DllSetRpcPrefixA := GetProcAddress(LibHandle, 'DllSetRpcPrefixA');
///      if Pointer(@DllSetRpcPrefix) = nil then RaiseLastOSError;
    except
...
procedure TBrowserInfo.SetCookieEx(const Name, Value, Expires, Path,
  Options: WideString);
begin
  if Assigned(FVirtualUI) then
    FVirtualUI.BrowserInfo.SetCookieEx(Name, Value, Expires, Path, Options);
end;
